...
for(minuto in c(4:297))
{
    ...
    write.csv(paste0("current,",minuto), file = paste0(args[2], "/current_engagement.txt"), row.names = FALSE)
    
    #pega minuto corrente e janela de 10 minutos ao redor (pegar menos dados se for jogos maiores)
    ...
    con = dbConnect(m, dbname=paste0(args[2], "/bases_sinteticas/dataset_eng.db"))
    dbExecute(con, "PRAGMA synchronous = OFF")
    dbExecute(con, "PRAGMA count_changes = OFF;")
    dbExecute(con, "PRAGMA locking_mode = EXCLUSIVE;")
