...
for(minuto in c(4:297))
{
    ...
    write.csv(paste0("current,",minuto), file = paste0(args[2], "/current_band_rand.txt"), row.names = FALSE)
    ...
    con = dbConnect(m, dbname=paste0(args[2], "/bases_sinteticas/dataset_band_rand.db"))
    dbExecute(con, "PRAGMA synchronous = OFF")
    dbExecute(con, "PRAGMA count_changes = OFF;")
    dbExecute(con, "PRAGMA locking_mode = EXCLUSIVE;")
    ...
    if(nrow(windowData) > 0)
    {
        ...
        ### AGORA ESTOU INTERESSADO NOS CLIENTES QUE IRÃO PERMANECER DAQUI A 1 MINUTO ### 
